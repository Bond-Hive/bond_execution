...
const getV6_2Monitoring = async function (strategy) {
  let resultObj = {};
...
const getHedgeTrancheDeploy = async function (tranche, strategy) {
  ...
  for (let cex in tranche.cex) {
    ...
    // let result = await getCexInfo(cexObj.cex, cexObj.cexSymbol, cexObj.cexSubaccount, apiKey, secret, password); // array response from ccxt fecth position
    // let result1 = await getCexInfo("binance", cexObj.cexSymbol, cexObj.cexSubaccount, apiKey, secret, password); // array response from ccxt fecth position
    const cexInitial = civfund.initializeCcxt("binance", cexObj.cexSubaccountBot);

    let tradingBotBalance = await getAccountBalance(cexInitial);
    // console.log("result",result);
    // console.log("result1",result1);
    let BTCPRICE = getWebSocketPriceMonitorUniversal().btcusdt.getPrice();
    const tradingBotsBalance = tradingBotBalance.find(wallet => wallet.walletName === 'Trading Bots').balance * BTCPRICE;
    const profitFromBot = cexObj.initialBOTCollateral - tradingBotsBalance - cexObj.initialCEXCollateralBTC * (cexObj.initialBTCprice - BTCPRICE);
    let {
    ...
    // hedge data
    let collateralFromOrders = JSON.parse(JSON.stringify(sumOfOrder)) * BTCPRICE; /*let collateralFromOrdersWNoise = JSON.parse(JSON.stringify(sumOfOrder));*/
    let sign = (side === "long") ? -1 : 1;

    collateralFromOrders += parseFloat(sumOfSellOrderQuantity) * markPrice * BTCPRICE;
    let cexName = 'cex' + cex;
    ...
    trancheObj[cexName] = {
      ...
      'collateralFromOrders': Number(collateralFromOrders) + Number(profitFromBot),
      'cexNAV': cexNAV,
      'markPrice': markPrice,
    }
  }
