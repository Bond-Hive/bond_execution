const { getCexInfo, OrdersV2, getAccountBalance } = require('./libraries/ccxtFunctions.js');
...
const getV6Monitoring = async function (strategy) {
  let resultObj = {};
...
const getHedgeTrancheDeploy = async function (tranche, strategy) {
  ...
  for (let cex in tranche.cex) {
    ...
    const tradingBotsBalance = tradingBotBalance.find(wallet => wallet.walletName === 'Trading Bots').balance * getWebSocketPriceMonitorUniversal().btcusdt.getPrice();
    const profitFromBot = cexObj.initialBOTCollateral - tradingBotsBalance;
    let {
