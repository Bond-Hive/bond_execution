...
const BigNumber = require('bignumber.js'); // Importing bignumber library. Ensure you have it in your project.
  ...
  try {
    if (method === "default") {
      if (tick < tickLower || tick > tickUpper) {
        ...
        fees0Token = (
          (Math.abs
            (Number(ticksUpperResults.feeGrowthOutside0X128) - Number(ticksLowerResults.feeGrowthOutside0X128)
            ) - feeGrowthInside0LastX128
          ) / (2 ** 128)
        ) * liquidity / (10 ** token0decimals);
        fees1Token = (
          (Math.abs(
            Number(ticksUpperResults.feeGrowthOutside1X128) - Number(ticksLowerResults.feeGrowthOutside1X128)
            ) - feeGrowthInside1LastX128
          ) / (2 ** 128)
        ) * liquidity / (10 ** token1decimals);
      } else {
        // When tokens are inside range
        fees0Token = (
          feeGrowthGlobal0X128 
          - Number(ticksLowerResults.feeGrowthOutside0X128) 
          - Number(ticksUpperResults.feeGrowthOutside0X128) 
          - feeGrowthInside0LastX128
        ) / (2 ** 128) * liquidity / (10 ** token0decimals);
        fees1Token = (
          feeGrowthGlobal1X128
          - Number(ticksLowerResults.feeGrowthOutside1X128)
          - Number(ticksUpperResults.feeGrowthOutside1X128)
          - feeGrowthInside1LastX128
        ) / (2 ** 128) * liquidity / (10 ** token1decimals);
      }
