...
const { getV2PairInfoWeb3, getV3PairInfoWeb3, getArbGMXPool } = require('./queries/web3Queries.js');
const civfund = require('@civfund/fund-libraries');
let feesCache = {};
const webSocketPriceMonitorUniversal = require('../../../streams/priceStreams.js'); // Price Websocket input for sharing  the particular asset prices
...
const getLpMonitoring = async function (strategy) {
  ...
  const CAGRWNoise = parseFloat(((((1 + (totalPercentChangeWNoise / 100)) ** (365 / daysDiff) - 1) * 100).toString()).split('e+')[0]);
  const totalPNLWNoise = totalValueWONoise - totalInitialValue;
...
const getV3LpTranche = async function (tranche, strategy) {
  ...
  console.log(uniV3UnclaimedFees);
    ...
    for (let cexId in tranche.cex) {
      ...
      if (cexObj.hedgeToken === token) {
        ...
        if (cexObj.cexSymbol == "BTC/USDT:USDT-240329"){
          ...
          const {
            info: CexInfo,
          } = await getCexInfo(cexObj.cex, cexObj.cexSymbol, cexObj.cexSubaccount, apiKey, secret, password); // array response from ccxt fetch position
          markPrice = CexInfo[0].info.markPrice;
          ...
          markPrice = cexObj.cexSymbol == "ARB/USDT" ? webSocketPriceMonitorUniversal.arbusdt.getPrice(): markPrice;
          markPrice = cexObj.cexSymbol == "BTC/USDT" ? webSocketPriceMonitorUniversal.btcusdt.getPrice(): markPrice;
        }
...
const getHedgeTrancheDeploy = async function (tranche, strategy) {
  ...
  for (let cex in tranche.cex) {
    ...
    if (!cexObj.calculationType) {
      ...
      } = await Orders(symbol, cexObj.tillDate, cexObj.cexSubaccount, cexObj.cex, cexObj.exchangeFees, apiKey, secret, password, stratOrderPrefix, orderNameStratControl);
    ...
    if (openPosition && !gammaHedge) { // To account for any open position. Note: openPosition can also exist in gammaHedge mode
      collateralFromOrders += (sign) * openPosition * markPrice;
      collateralFromOrdersWNoise += (sign) * openPosition * dexPrice;
    }
  ...
  if (numberOfObjects === 1) {
    ...
    console.log("Monitoring doesn't support more than 2 CEX objects");
    console.error("Monitoring doesn't support more than 2 CEX objects");
  }
